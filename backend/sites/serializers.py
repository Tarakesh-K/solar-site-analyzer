...
class SiteSerializer(serializers.ModelSerializer):
    ...
    def validate_land_type(self, value):
        valid_types = ['Agricultural', 'Industrial', 'Wasteland', 'Hilly', 'Open Land', 'Mixed Use', 'Forest Adjacent', 'Near Water Body', 'Peri-Urban']
    ...
    def validate_area_sqm(self, value):
        if value < 500: # Minimum logic: anything smaller isn't a "site"
            raise serializers.ValidationError("Area is too small for solar development (min 500 sqm).")
        return value

    def validate_solar_irradiance_kwh(self, value):
        if not (0 <= value <= 10): # Physical limit: even the Sahara doesn't exceed 8-9
            raise serializers.ValidationError("Solar irradiance must be between 0 and 10 kWh/m²/day.")
        return value
    ...
    def validate_grid_distance_km(self, value):
        ...
        if value > 500:
            raise serializers.ValidationError("Distance exceeds 500km; site is likely too remote.")
        return value

    def validate_slope_degrees(self, value):
        if not (0 <= value <= 90):
            raise serializers.ValidationError("Slope degrees must be between 0 (flat) and 90 (vertical).")
        return value
    ...
    def validate_elevation_m(self, value):
        ...
        if not (-413 <= value <= 8848):
            raise serializers.ValidationError("Elevation must be between -413m (Dead Sea) and 8848m (Everest).")
        return value
...
class CalculatedAnalysisSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        # Apply Formulas
        s_score = 100.0 if irr >= 5.5 else (0.0 if irr < 3.0 else ((irr - 3.0) / 2.5) * 100)
        a_score = 100.0 if area >= 50000 else (0.0 if area < 5000 else ((area - 5000) / 45000) * 100)
        g_score = 100.0 if dist <= 1 else (0.0 if dist >= 20 else 100 - ((dist - 1) / 19) * 100)
        i_score = 100.0 if road <= 0.5 else (0.0 if road >= 5 else 100 - ((road - 0.5) / 4.5) * 100)

        if slope <= 5: sl_score = 100.0
        elif slope > 20: sl_score = 0.0
        elif slope <= 15: sl_score = 100 - ((slope - 5) / 10) * 50
        else: sl_score = 50 - ((slope - 15) / 5) * 50

        total = (s_score * weights['solar']) + (a_score * weights['area']) + \
                (g_score * weights['grid']) + (sl_score * weights['slope']) + \
...
class SiteDetailSerializer(serializers.ModelSerializer):
    analysis_history = AnalysisResultSerializer(many=True, read_only=True, source='analysis_results')
